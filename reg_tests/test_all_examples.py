...
class TestExamples(unittest.TestCase):
    ...
    def test_2D_euler(self):
        from pyhyp_examples.naca0012.naca0012_euler import volumeFile, hyp

        marchDist = hyp.getOption("marchDist")
        self.common_test(volumeFile, marchDist)

    def test_2D_rans(self):
        from pyhyp_examples.naca0012.naca0012_rans import volumeFile, hyp

        marchDist = hyp.getOption("marchDist")
        self.common_test(volumeFile, marchDist)

    def test_717(self):
        from pyhyp_examples.wing_717.run717 import volumeFile, hyp

        marchDist = hyp.getOption("marchDist")
        self.common_test(volumeFile, marchDist)

    def test_BWB(self):
        from pyhyp_examples.BWB.runBWB import volumeFile, hyp

        marchDist = hyp.getOption("marchDist")
        self.common_test(volumeFile, marchDist)

    def test_corner(self):
        from pyhyp_examples.corner.runCorner import volumeFile, commonOptions

        marchDist = commonOptions["marchDist"]
        self.common_test(volumeFile, marchDist)

    def test_M6(self):
        from pyhyp_examples.m6.runM6 import volumeFile, hyp

        marchDist = hyp.getOption("marchDist")
        self.common_test(volumeFile, marchDist)

    def test_plate(self):
        from pyhyp_examples.plate.runPlate import volumeFile, hyp

        marchDist = hyp.getOption("marchDist")
        self.common_test(volumeFile, marchDist)

    def test_sphere(self):
        from pyhyp_examples.sphere.runSphere import volumeFile, commonOptions
